...
public class CategorieSubCatSub {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        String catSub_sub = "parentSub_sub.csv";
        List<String> catString = getFirstColumn(catSub);
        List<String> parentCatString = getFirstColumn(parentCat);
        try (FileWriter writer = new FileWriter(catSub_sub)) {
            ...
            for (Element categorie : products) {
                ...
                for (Element subcat : subCats) {
                    Elements sub_subCats = categorie.select(".vertical_fullwidthmenu.varticalmenu_main.fullwidth.clearfix").select(".level3-popup.halfwidth-popup-sub-sub");
                    for (Element sub_subcat : sub_subCats) {
                        for (Element elem : sub_subcat.getElementsByTag("li")) {
                            writer.append("1,");
                            writer.append(elem.text() + ",");
                            System.out.println(indexCat_sub);
                            writer.append(catString.get(indexCat_sub) + ",");
                            writer.append(elem.text() + "1,");
